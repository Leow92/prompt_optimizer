"""Prompt optimization utility using Groq or OpenAI LLMs."""
...
def set_client(provider: str, base_url: str = None):
    ...
    if provider == "groq":
        ...
        raise ValueError(
            f"❌ Unsupported provider: {provider}. Use 'groq' or 'openai'."
        )
...
def optimize_prompt(
    ...
    if _client is None or _provider is None:
        raise RuntimeError(
            "❌ No client set. Use set_client('groq') or set_client('openai') first."
        )

    messages = [{"role": "system", "content": SYSTEM_PROMPT_V02}]

    messages.append(
        {"role": "user", "content": f"Prompt draft of the user:\n{prompt_draft}"}
    )

    if user_input_example:
        messages.append(
            {"role": "user", "content": f"Example of User input:\n{user_input_example}"}
        )

    if llm_output_example:
        messages.append(
            {"role": "user", "content": f"Example of Output:\n{llm_output_example}"}
        )
    ...
    if top_instruction:
        messages.append(
            {
                "role": "user",
                "content": f"Specific extra context provided by the user:\n{top_instruction}",
            }
        )
